...
const Home = () => {
  ...
  const few = news.slice(0, SHOW_COUNT);

  return (
    <main className="container mx-auto px-4 py-6">
      <h1 className="text-xl font-bold mb-4">Home</h1>

      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6">
        {few.map((n) => {
          const {
            id,
            title = "Untitled",
          ...
          return (
            <article
              key={id}
              className="border border-gray-300 bg-white shadow-sm overflow-hidden rounded-xl"
            >
              {/* HEADER */}
              ...
              <div className="px-4 py-3 text-slate-700 text-sm">
                <p className="line-clamp-3" title={details}>
                  {details}
                </p>
                <Link
                  to={`/news/${id}`}
